...
pub fn insert_picked_movement<S0: ::std::hash::BuildHasher, S1: ::std::hash::BuildHasher, S2: ::std::hash::BuildHasher, S3: ::std::hash::BuildHasher>(
    ...
    for dan_dst in 1..10 {
        for suji_dst in 1..10 {
            ...
            if let Koma::Kara = km_dst {
                // 駒が無いところに打つ

                hashset_drop.clear();
                for kms_motigoma in &MGS_ARRAY {
...
pub fn insert_narazu_src_by_sn_ms<S: ::std::hash::BuildHasher>(
    ...
    for kms in &KMS_ARRAY {
        ...
        for i_dir in 0..KM_UGOKI_LN{ // 指定の駒種類の、全ての逆向きに動ける方向
            let _kmdir;
            let p_kmdir : &KmDir = if Sengo::Sen == sn {
                // g_writeln(&format!("get_src_by_sn_ms 先手なら kms={} kms_num={} p_kmdir={}",
                //     kms, kms_num, p_kmdir
                // ));
                &KM_UGOKI.back[kms_num][i_dir]
            } else {
                _kmdir = hanten_kmdir_joge(&KM_UGOKI.back[kms_num][i_dir]);
